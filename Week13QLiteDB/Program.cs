...
{
    SQLiteConnection connection = new SQLiteConnection("Data Source=mydb.db; Versiom = 3; New = True; Compress = True;");
...
static void FindCustomer(SQLiteConnection myConnection)
{
    ...
    command = myConnection.CreateCommand();
    command.CommandText = $"SELECT customer.customerId, customer.firstName, customer.lastName, status.statusId " +
        $"From customerStatus " +
